                ...
                for pkg_name, pkg_res in bsp_res.items():
                    ...
                    for version_res in pkg_res['versions']:
                        if version_res['version'] == 'latest':
                            ...
                            print('check {pkg} {version} on {bsp}.'.format(pkg=pkg_name, version=version_res['version'], bsp=bsp_name))
                            ...
                            print('check {pkg} {version} on {bsp}. but not is latest, pass.'.format(pkg=pkg_name, version=version_res['version'], bsp=bsp_name))
                    if check_counter == 0:
                        error_num = error_num + 1
